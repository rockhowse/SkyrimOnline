	...
	namespace Game
	{
			...
			: mInit(false)
		{
			...
			mMe = boost::make_shared<FreeScript::Character>(
				(Actor*)FreeScript::ObjectReference::PlaceAtMe((TESObjectREFR*)FreeScript::Game::GetPlayer(), me, 1, true, false));
			mMaster = (Actor*)::ObjectReference::PlaceAtMe((TESObjectREFR*)FreeScript::Game::GetPlayer(), me, 1, true, true);

			FreeScript::SActor::EnableAI(mMe->GetActor(), true);
			FreeScript::SActor::KeepOffsetFromActor(mMe->GetActor(), mMaster, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 20.f, 5.0f);
			FreeScript::SActor::SetActorValue(mMe->GetActor(), "Health", 99999999.f);
		...
		ActorController::~ActorController()
		{
			...
			FreeScript::ObjectReference::Delete((TESObjectREFR*)mMe->GetActor());
			FreeScript::ObjectReference::Delete((TESObjectREFR*)mMaster);
		...
		{
			FreeScript::ObjectReference::SetPosition((TESObjectREFR*)mMaster, x,y,z);
			mMe->SetPos(x,y,z);
			if(mMount)
				mMount->SetPos(x,y,z);
		}
		...
		{
			FreeScript::ObjectReference::SetAngle((TESObjectREFR*)mMaster, x,y,z);
			mMe->SetRot(x,y,z);
			if(mMount)
				mMount->SetRot(x,y,z);
		}
		...
		{
			if(!mInit || FreeScript::ObjectReference::GetDistance((TESObjectREFR*)mMaster, (TESObjectREFR*)mMe->GetActor()) > 600.f)
			{
			...
			FreeScript::ObjectReference::SetPosition((TESObjectREFR*)mMaster, posX,posY,posZ);
			FreeScript::ObjectReference::SetAngle((TESObjectREFR*)mMaster, rotX,rotY,rotZ);
		}
		//--------------------------------------------------------------------------------
		void ActorController::SetMount(uint32_t pMount)
		{
			if(mMount)
			{
				...
				FreeScript::ObjectReference::Delete((TESObjectREFR*)mMount->GetActor());
				mMount.reset();
			}
			if(pMount != 0)
			{
				Actor* form =  rtti_cast(FreeScript::ObjectReference::PlaceAtMe((TESObjectREFR*)FreeScript::Game::GetPlayer(), FreeScript::Game::GetForm(pMount), 1, true, false), TESObjectREFR, Actor);
				...
				FreeScript::SActor::EnableAI(mMount->GetActor(), true);

//				ObjectReference::TetherToHorse((TESObjectREFR*)mMe->GetActor(), (TESObjectREFR*)mMount->GetActor());
				FreeScript::SActor::KeepOffsetFromActor(mMount->GetActor(), mMaster, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 10.f, 3.f);
