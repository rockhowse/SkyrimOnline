...
namespace Skyrim
{
	namespace Game
	{
		...
		TESForm* SkyrimFormManager::GetForm(uint32_t pRace, uint32_t pGender)
		{
			...
			for(auto i = 0; i < 5 && std::find(mUsed.begin(), mUsed.end(), actorId) != mUsed.end(); ++i)
			{
				--actorId;
			}

			TESForm* baseForm = FreeScript::Game::GetForm(actorId);

			mUsed.push_back(actorId);
