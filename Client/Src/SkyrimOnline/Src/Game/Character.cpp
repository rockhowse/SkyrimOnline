...
namespace Skyrim
{
	namespace Game
	{
		...
		{
			return Form::GetFormID((TESForm*)dyn_cast(Actor::GetRace(mActor), "TESRace", "TESForm"));
		}
		//--------------------------------------------------------------------------------
		uint32_t Character::GetGender()
		{
			return ActorBase::GetSex((TESNPC*)dyn_cast(SkyrimScript::GetBaseForm(mActor), "TESForm", "TESNPC"));
		}
		//--------------------------------------------------------------------------------
		const std::vector<float>& Character::GetFaceMorph()
		{
			SkyrimScript::GetFaceMorph((TESNPC*)dyn_cast(SkyrimScript::GetBaseForm(mActor), "TESForm", "TESNPC"), &mFaceMorph);
			return mFaceMorph;
		}
		//--------------------------------------------------------------------------------
		void Character::SetFaceMorph(const std::vector<float>& pFaceMorphs)
		{
			...
			SkyrimScript::SetFaceMorph((TESNPC*)dyn_cast(SkyrimScript::GetBaseForm(mActor), "TESForm", "TESNPC"), &mFaceMorph);
